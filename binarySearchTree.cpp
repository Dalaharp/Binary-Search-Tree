...
Node *join(Node *L, Node *R)
{
    ...
    // Determine the root of the joined tree with probability proportional to sizes
...
Node *remove(Node *T, int k)
{
    ...
    if (k < T->key) {
        ...
        // Node with two children, get the in-order successor (min node from the right subtree)
        Node *temp = T->right;
...
{
    if (T == nullptr) return new Node(k);
    int N = (T->size) ? T->size : 1; // In case size is not updated correctly
    if (rand() % N == 0) {
        // Insert k at the root with probability 1/N
